...
$signup = @{ email = $Email; password = $Password; nickname = $Nickname } | ConvertTo-Json
try {
    Invoke-RestMethod -Uri "$Api/api/auth/signup" -Method Post `
        -ContentType 'application/json; charset=utf-8' -Body $signup | Out-Null
} catch {
...
try {
    $token = (Invoke-RestMethod -Uri "$Api/api/auth/login" -Method Post `
        -ContentType 'application/json; charset=utf-8' -Body $login).data.accessToken
} catch {
    Bad "로그인 실패. 이미 다른 비밀번호로 가입된 계정일 수 있습니다."
    Write-Host "     `$env:TEST_EMAIL='다른주소'; 로 지정한 뒤 다시 시도하세요."
    exit 1
}
Ok "로그인 완료 ($Email)"
...
Write-Host "  $WaitSeconds초 동안 기다립니다..."
Write-Host ""
